    ...
    sendRequest: (options, callback, onData) => {
        ...
        if (!stream) {
            ...
            Object.keys(options.agent.requests).forEach((connectionName) => {
                console.debug("[INFO] Socket pool for", connectionName, "has", options.agent.requests[connectionName].length, "pending requests over", options.agent.sockets[connectionName].length, "sockets")
            })
        }

        console.debug("[INFO] HTTPS OUT", options.hostname, options.port, options.method, options.path)
            ...
            options.onResponse ||
                function (response) {
                    ...
                    response.once("end", () => {
                        ...
                        console.debug("[INFO]  HTTPS IN ", options.hostname, options.port, options.method, options.path)
                        if (options.agent) {
                            Object.keys(options.agent.requests).forEach((connectionName) => {
                                console.debug("[INFO] Socket pool for", connectionName, "has", options.agent.requests[connectionName].length, "pending requests over", options.agent.sockets[connectionName].length, "sockets")
                            })
