const util = require("util") // .promisify() Added in Node.js v8.0.0
...
class OANDAAdapterV20 {
    ...
    getPrice(symbol, callback) {
        if (!this.accountId) {
            this.getAccountId((error) => (error ? callback(error) : this.getPrice(symbol, callback)))
            return
        }
        ...
        this._requestHTTP(
            {
                method: "GET",
                path: `/v3/accounts/${this.accountId}/pricing?instruments=` + symbol
            },
            (error, body, statusCode) => {
                if (error) {
                    if (body && body.message) {
    ...
    subscribePrice(accountId, symbol, listener, context) {
        if (!accountId && !this.accountId) {
            this.getAccountId((error) => error || this.subscribePrice(accountId, symbol, listener, context))
            return
        }
