...
class OANDAAdapterV20 {
    ...
    getInstruments(accountId, callback) {
        if (!accountId) {
            this.getAccountId((error, accountId) => (error ? callback(error) : this.getInstruments(accountId, callback)))
            return
        }
        this._requestHTTP(
            {
    ...
    getPrice(accountId, symbol, callback) {
        if (!accountId) {
            this.getAccountId((error, accountId) => (error ? callback(error) : this.getPrice(accountId, symbol, callback)))
            return
        }
    ...
    getCandles(accountId, instrument, from, to, interval, callback) {
        ...
        this._requestHTTP(
            {
                method: "GET",
                path: `/v3/accounts/${accountId}/instruments/${instrument}/candles?price=BMA&granularity=${interval}&from=${from}&to=${to}`
            },
            (error, body, statusCode) => {
                if (error) {
                    if (body && body.errorMessage) {
